        ...
        try:
            with _cursor() as cur:
                ...
                fvg_data = {
                    ...
                    "entry_trigger": signal.indicators.get("entry_trigger", "retest"),
                    "entry_mode": signal.indicators.get("entry_mode", "close"),
                    "retest_enabled": bool(signal.indicators.get("retest_enabled", 0.0)),
                    "supertrend_trend": int(signal.indicators.get("supertrend_trend", 0) or 0),
                    "supertrend_band": float(signal.indicators.get("supertrend_band", 0.0) or 0.0),
                    "supertrend_switch_price": float(signal.indicators.get("supertrend_switch_price", 0.0) or 0.0),
                ...
                cur.execute(
                    ...
                    (
                        ...
                        float(signal.entry),
                        "v2",
                        f"v2 {signal.direction_str.upper()} FVG RETEST",
                        True,
                        signal.direction_str,
                        "v2 FVG retest + SuperTrend",
                        signal.direction_str,
                        float(signal.entry),
                        float(signal.sl),
                        tp1,
